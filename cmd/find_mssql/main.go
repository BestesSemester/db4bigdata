...
func main() {
	...
	pm := performancemeasurement.New(model.MSQL, "find_mssql")
	...
	db.Model(&invoice_t).Where("Invoice_Date between ? and ?", startDate, endDate).Distinct().Pluck("Agent_ID", &agents)
	prov_delete := &model.ProvisionDistribution{}
	db.Where("provision_part > 0").Delete(&prov_delete)
